...
export function StockAlertManager() {
    ...
    const sendStockAlert = useCallback(async (product: typeof STOCK_PRODUCTS[0], type: 'rupture' | 'retour') => {
        ...
        try {
            ...
            console.error('[Stock] Broadcast error:', e);
        }

        // Local confirmation
        setIncomingAlert(alert);
        playStockSound();
        setTimeout(() => setIncomingAlert(prev => prev === alert ? null : prev), 5000);
    }, [myDeviceId]);

    useEffect(() => {
        if (!supabase) return;

        const channel = supabase.channel(`${BROADCAST_CHANNEL}_listen`)
            .on('broadcast', { event: 'stock_change' }, (payload: any) => {
                const alert = payload.payload as StockAlert;
                if (!alert || alert.fromDevice === myDeviceId) return;

                console.log('[Stock] Received:', alert.type, alert.productName);
                setIncomingAlert(alert);
                playStockSound();
            })
            .subscribe();
    ...
    return createPortal(
        <>
            {/* FLOATING RUPTURE BUTTON — always visible */}
            <button
                onClick={() => setIsPanelOpen(true)}
                className="fixed bottom-6 right-6 z-[9999] px-5 py-3 bg-amber-500/90 hover:bg-amber-500 text-black font-black text-sm rounded-full shadow-2xl shadow-amber-500/30 active:scale-95 transition-all flex items-center gap-2 backdrop-blur-lg border border-amber-400/50"
            >
                <Package size={18} />
                RUPTURE
            </button>
            ...
            {isPanelOpen && (
                ...
                >
                    <div className="w-full max-w-2xl max-h-[90vh] bg-zinc-950 border border-zinc-800 rounded-t-3xl sm:rounded-3xl overflow-hidden flex flex-col shadow-2xl">
                        <div className="p-5 border-b border-zinc-800 flex items-center justify-between bg-amber-500/10">
                            ...
                            <button onClick={() => setIsPanelOpen(false)} className="p-3 hover:bg-white/10 rounded-full">
                                ...
                                {STOCK_PRODUCTS.map((product) => (
                                    <div key={product.id} className="flex flex-col gap-1.5">
                                        <button
                                            ...
                                            className="aspect-[4/3] rounded-2xl bg-red-600/20 hover:bg-red-600/40 border-2 border-red-500/30 hover:border-red-500 flex flex-col items-center justify-center gap-2 transition-all active:scale-95"
                                        >
                                        ...
                                        <button
                                            ...
                                            className="py-2 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 hover:border-emerald-500/40 text-emerald-400 text-[11px] font-bold uppercase tracking-wide flex items-center justify-center gap-1 active:scale-95"
                                        >
                                            <CheckCircle size={12} /> Retour
                                        </button>
                                    </div>
                                ))}
                            </div>
            ...
            {/* INCOMING STOCK ALERT BANNER */}
            {incomingAlert && (
                <div
                    className={`fixed top-0 left-0 right-0 z-[99999] cursor-pointer ${isRupture ? 'bg-amber-500' : 'bg-emerald-500'}`}
                    onClick={() => setIncomingAlert(null)}
                >
                    <div className="w-full max-w-3xl mx-auto flex items-center gap-4 py-5 px-6">
                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 ${isRupture ? 'bg-black/30' : 'bg-black/20'}`}>
                            {isRupture
                        ...
                        <div className="flex-1 text-white">
                            <h2 className="text-2xl md:text-3xl font-black uppercase">
                                {isRupture ? '⛔ RUPTURE' : '✅ RETOUR'}
                            </h2>
                            <p className="text-lg md:text-xl font-bold text-white/90">{incomingAlert.productName}</p>
                            <p className="text-xs font-medium text-white/60">par {incomingAlert.fromDevice} · Cliquez pour fermer</p>
                        </div>
                        <button onClick={() => setIncomingAlert(null)} className="p-2 bg-black/20 hover:bg-black/40 rounded-full flex-shrink-0">
